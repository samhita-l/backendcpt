...
class Post(db.Model):
    ...
    def __init__(self, id, name, note, image):
        self.userID = id
        self.name = name
        self.note = note
        self.image = image
...
class User(db.Model):
    ...
    _cost = db.Column(db.Integer, nullable = False)
    _weather = db.Column(db.JSON, nullable = False)
        ...
        for user in users:
            try:
                ...
                for num in range(randrange(1, 4)):
                    ...
                    user.posts.append(Post(id=user.id, note=note, name=user.name, image='ncs_logo.png'))
